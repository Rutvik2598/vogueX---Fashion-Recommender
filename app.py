...
class ForgotPasswordForm(FlaskForm):
    ...
    new_password = PasswordField(
        "New Password", validators=[InputRequired(), Length(min=8, max=80)]
    )
...
def create_app(config_name="default"):
    ...
    def process_glasses():
        ...
        glasses = cv.imdecode(
            np.frombuffer(glasses_file.read(), np.uint8), cv.IMREAD_UNCHANGED
        )
        ...
        face_cascade = cv.CascadeClassifier(
            "static\cascades\haarcascade_frontalface_alt.xml"
        )
        ...
        return render_template(
            "tryon_result.html", image=processed_image_base64, tryon_type="Glass"
        )
    ...
    def process_necklace():
        ...
        necklace = cv.imdecode(
            np.frombuffer(necklace_file.read(), np.uint8), cv.IMREAD_UNCHANGED
        )
        ...
        def acc_cor_points(oimage):
            ...
            second_half = oimage[
                0 : oimage.shape[1], int(oimage.shape[0] / 2) : oimage.shape[0]
            ]
        ...
        predictor = dlib.shape_predictor(
            "static\misc\shape_predictor_68_face_landmarks.dat"
        )
        ...
        for i, rect in enumerate(rects):
            ...
            resized1 = cv.resize(
                resized,
                (resized.shape[1] * 10, resized.shape[0] * 10),
            ...
            roi = cv.resize(
                roi1,
                (roi1.shape[1] * 10, roi1.shape[0] * 10),
                interpolation=cv.INTER_AREA,
            )
            ...
            dst = cv.resize(
                dst1,
                (int(dst1.shape[1] / 10), int(dst1.shape[0] / 10)),
                interpolation=cv.INTER_AREA,
            )
        ...
        return render_template(
            "tryon_result.html", image=processed_image_base64, tryon_type="Necklace"
        )
    ...
    def process_earrings():
        ...
        image = cv.imdecode(nparr, cv.IMREAD_UNCHANGED)
        left_ear = cv.imdecode(
            np.frombuffer(lear_file.read(), np.uint8), cv.IMREAD_UNCHANGED
        )
        right_ear = cv.imdecode(
            np.frombuffer(rear_file.read(), np.uint8), cv.IMREAD_UNCHANGED
        )
            ...
            for i in range(left_earring_resized.shape[0]):
                for j in range(left_earring_resized.shape[1]):
                    if left_earring_resized[i, j, 3] != 0:
                        image[left_ear_y + i, left_ear_x + j] = left_earring_resized[
                            i, j, :3
                        ]
            ...
            for i in range(right_earring_resized.shape[0]):
                for j in range(right_earring_resized.shape[1]):
                    if right_earring_resized[i, j, 3] != 0:
                        image[right_ear_y + i, right_ear_x + j] = right_earring_resized[
                            i, j, :3
                        ]
        else:
        ...
        return render_template(
            "tryon_result.html", image=processed_image_base64, tryon_type="Earrings"
        )
    ...
    def process_lips():
        ...
        return render_template(
            "tryon_result.html", image=processed_image_base64, tryon_type="Lip Color"
        )
    ...
    def process_hairstyle():
        ...
        image = cv.imdecode(nparr, cv.IMREAD_UNCHANGED)
        hairstyle = cv.imdecode(
            np.frombuffer(hairstyle_file.read(), np.uint8), cv.IMREAD_UNCHANGED
        )
        ...
        processed_image_path = (
            "hairstyle-try-on/exp/image_samples/images/original_input.png"
        )
        ...
        return render_template(
            "tryon_result.html", image=processed_image_base64, tryon_type="Hairstyle"
        )
