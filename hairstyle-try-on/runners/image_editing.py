...
class Diffusion(object):
    def __init__(
        self, image_folder, sample_step, total_noise_levels, config, device=None
    ):
        # self.args = args
        ...
        ckpt = torch.hub.load_state_dict_from_url(
            url, model_dir="checkpoints/", map_location=self.device
        )
    ...
    def image_editing_sample(self, img, mask):
        ...
        with torch.no_grad():
            ...
            img = torch.tensor(img / 255.0).to(dtype=torch.float32)
            ...
            tvu.save_image(x0, os.path.join(self.image_folder, "original_input.png"))
            tvu.save_image(mask, os.path.join(self.image_folder, "mask.png"))
            x0 = (x0 - 0.5) * 2.0
            ...
            for it in range(self.sample_step):
                ...
                tvu.save_image(
                    (x + 1) * 0.5,
                    os.path.join(self.image_folder, f"init_{ckpt_id}.png"),
                )

                with tqdm(
                    total=total_noise_levels, desc="Iteration {}".format(it)
                ) as progress_bar:
                    for i in reversed(range(total_noise_levels)):
                        ...
                        if (i - 99) % 100 == 0:
                            tvu.save_image(
                                (x + 1) * 0.5,
                                os.path.join(
                                    self.image_folder, f"noise_t_{i}_{it}.png"
                                ),
                            )
                        progress_bar.update(1)

                x0[:, (mask != 1.0)] = x[:, (mask != 1.0)]
                torch.save(x, os.path.join(self.image_folder, f"samples_{it}.pth"))
                tvu.save_image(
                    (x + 1) * 0.5, os.path.join(self.image_folder, f"samples_{it}.png")
                )
        ...
        with torch.no_grad():
            ...
            for it in range(self.sample_step):
                ...
                with tqdm(
                    ...
                    for i, st_progress in zip(
                        reversed(range(total_noise_levels)), range(total_noise_levels)
                    ):
                        t = (torch.ones(n) * i).to(self.device)
                        x_ = image_editing_denoising_step_flexible_mask(
                            x,
                        ...
                        if (i - 99) % 100 == 0:
                            ...
                            self.show_images[f"noise_t_{i}_{it}"] = self.to_cv2image(
                                (x + 1) * 0.5
                            )
                        progress_bar.update(1)
                        my_bar.progress(
                            int((st_progress) * 100 / total_noise_levels)
                            + int(100 / total_noise_levels)
                        )
