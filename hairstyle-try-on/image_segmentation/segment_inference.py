...
from image_segmentation.utils import label_to_image, load_checkpoint
...
class face_segment:
    def __init__(
        ...
        self.transform = A.Compose(
            [
                A.Resize(
                    height=self.input_image_size[0], width=self.input_image_size[1]
                ),
                A.Normalize(
                    mean=[0.0, 0.0, 0.0],
                    std=[1.0, 1.0, 1.0],
        ...
        checkpoint = torch.load(
            os.path.join(self.seg_model_path), map_location=torch.device("cpu")
        )
