...
def train_unet_parse_args():
    ...
    parser.add_argument(
        "--image_height", type=int, default=256, help="output image height"
    )
    parser.add_argument(
        "--image_width", type=int, default=256, help="output image width"
    )
    parser.add_argument("--pin_memory", type=bool, default=True, help="pin_memory")
    parser.add_argument(
        "--load_model", type=bool, default=False, help="load exist model"
    )
    parser.add_argument(
        "--model_checkpoint_path",
        type=str,
        default="",
        help="if load_model load from this path",
    )
    parser.add_argument(
        "--save_model_freq", type=int, default=1, help="save model every ... epoch"
    )
    parser.add_argument(
        "--image_dir",
        type=str,
        default=os.path.join("CelebAMask-HQ", "CelebA-HQ-img"),
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            "CelebAMask-HQ", "CelebAMask-HQ-mask-anno", "multiclass_dataset"
        ),
        help="path to mask directory",
    )
    parser.add_argument(
        "--save_sample_image_dir",
        type=str,
        default=os.path.join("image_segmentation", "saved_images"),
        help="path to save sample image",
    )
    parser.add_argument(
        "--save_model_checkpoint_path",
        type=str,
        default=os.path.join("image_segmentation", "my_checkpoint.pth.tar"),
        help="path to new model checkpoint",
    )
    parser.add_argument(
        "--train_test_split_size", type=float, default=0.9, help="train size"
    )
    parser.add_argument(
        "--save_sample_image_freq",
        type=int,
        default=1,
        help="save sample image each ... epoch",
    )
...
def train_fn(loader, model, optimizer, loss_fn, scaler):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        )  # targets.float().unsqueeze(1).to(device=DEVICE, dtype=torch.int64)
    ...
    for epoch in range(args.n_epochs):
        ...
        if (epoch + 1) % args.save_sample_image_freq == 0:
            save_predictions_as_imgs(
                val_loader,
                model,
                folder=args.save_sample_image_dir,
