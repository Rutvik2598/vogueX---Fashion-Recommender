...
def background_artifact_fill(
    original_image, original_mask, new_image, background_radius
):
    ...
    filled_background = cv2.inpaint(
        original_image, dilation_mask, background_radius, cv2.INPAINT_TELEA
    )
    filled_background = np.where(
        new_image == np.array([255, 255, 255]), filled_background, new_image
    )

    return filled_background


def face_artifact_fill(
    original_image,
    original_mask,
    new_image,
    new_mask,
    ...
    diff = np.where(
        (convert_to_gray(new_mask) == convert_to_gray(new_segment)), 0, 255
    ).astype("uint8")
    filled_face_artifact = cv2.inpaint(
        filled_bg_artifact, diff, face_radius, cv2.INPAINT_TELEA
    )
