...
def get_xy_coordinates(results, image_size):
    ...
    point_lm_index = [
        ...
        386,
        152,
    ]  # [10, 159, 386, 152] # [10, 159, 386]  # Face landmark position
    ...
    return (
        np.array(x_coordinates)[point_lm_index],
        np.array(y_coordinates)[point_lm_index],
    )
...
def get_landmark_coordinates(image):
    with mp_face_mesh.FaceMesh(
        ...
        landmark_coordinates = np.array(landmark_coordinates).astype(np.float32)
...
def transform_process(image, matrix):
    ...
    return cv2.warpPerspective(
        src=image, M=matrix, dsize=(image.shape[1], image.shape[0])
    )


def crop_image_by_mask(method, image, mask, color):
    if method == "hair_only":
        generate_mask = 255 * np.ones((mask.shape), dtype=int)
        generate_mask[(mask == np.array([255, 0, 0])).all(axis=2)] = 0  # Red only
        croped_image = np.where(generate_mask == 0, image, color)
        croped_mask = np.where(generate_mask == 0, mask, 0)
    elif method == "no_hair":
        generate_mask = np.zeros((mask.shape), np.uint8)
        generate_mask[
            (mask == np.array([255, 0, 0])).all(axis=2)
            | (mask == np.array([0, 0, 0])).all(axis=2)
        ] = 255  # Red and black
...
def merge_head_part(type, hair, face, color=[]):
    if type == "image":
        ...
        head = face.copy()
        head[(hair == np.array([255, 0, 0])).all(axis=2)] = np.array([255, 0, 0])
        return head
...
def face_landmark_transform(
    static_image, static_mask, transform_image, transform_mask, background="white"
):
    background_color_list = background_color_handle(background)

    static_landmark_coordinates, transform_landmark_coordinates = (
        get_landmark_coordinates(static_image),
        get_landmark_coordinates(transform_image),
    )
    ...
    ), transform_process(transform_mask, transform_matrix)
    ...
    cv2.imwrite(
        "report_images/only_fixed_face_color.png",
        cv2.cvtColor(only_fixed_face, cv2.COLOR_RGB2BGR),
    )
    t = color_mask_to_binary_mask_tensor(only_fixed_face.copy())
    cv2.imwrite(
        "report_images/only_fixed_face_binary.png", cv2.cvtColor(t, cv2.COLOR_RGB2BGR)
    )
