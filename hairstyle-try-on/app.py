...
from inference import dict2namespace, resize_image
...
class app:
    def __init__(self):
        ...
        run = st.button("RUN")
        if (
            self.args["target_image"] and self.args["source_image"] and run
        ):  # run pipeline when input images
            self.pipeline()
    ...
    def init_app(
        ...
        if self.args["target_image"] and self.args["source_image"]:
            self.target_image = self.read_image_from_streamlit(
                self.args["target_image"]
            )
            self.source_image = self.read_image_from_streamlit(
        ...
        self.args["sample_step"] = st.sidebar.number_input(
            "Total sampling steps (Number of generated images)",
            min_value=0,
            max_value=5,
            value=1,
            step=1,
        ...
        self.args["t"] = st.sidebar.number_input(
            "Sampling noise scale (Too much is slower, but too little results in an unsatisfying.)",
            min_value=0,
            max_value=2000,
            value=500,
            step=1,
    ...
    def create_args(self):
        ...
        args["input_image_size"] = (
            256,
            256,
        )  # input image size before segment (height, width)
        args["label_config"] = os.path.join(
            "image_segmentation", "label.yml"
        )  # Path to the label.yml
    ...
    def pipeline(
        ...
        transform_outputs = face_landmark_transform(
            target_image, target_mask, source_image, source_mask
        )
        transformed_image, transformed_mask = (
            transform_outputs["result_image"],
            transform_outputs["result_mask"],
        )
        ...
        try:
            ...
            for it in range(self.args["sample_step"]):
                st.image(
                    self.show_images[f"samples_{it}"],
                    width=300,
                    caption=f"Final image {it+1}",
                    clamp=True,
                )
